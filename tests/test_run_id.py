...
def test_derive_run_id_cross_machine_golden() -> None:
    ...
    # Golden rotated by each schema addition (LLR-0031: any config field
    # change must surface as a new run_id so partials don't get silently
    # cross-loaded between incompatible configs):
    ...
    #   - Phase 7.2 (Run-2 bundle) added `kd_topk`, `muon_*` fields,
    #     `adamw_group_learning_rate`, `warmup_sequence_length`, and
    #     `plateau_guard_*` fields; literals on `optimizer` and
    #     `trainable_scope` were extended. Result:
    #     `94261061b2d9bda0` → `439661d1048150ea`.
    # Verified: no in-flight vast.ai runs against the prior hashes
    # (Phase 7.1 smoke completed, Run-1 collapsed and destroyed).
    assert rid == "439661d1048150ea", (
